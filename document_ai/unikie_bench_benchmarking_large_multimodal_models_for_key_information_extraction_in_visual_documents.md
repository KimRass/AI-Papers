...
**Q. LMM이 KIE를 수행할 때 내부적으로 어떻게 작동하는가? (Appendix A.8)**

Appendix A.8 "Behavioral Analysis for LMMs in KIE". Qwen3-VL-8B-Instruct를 대표 모델로 선택해 레이어별 attention 패턴을 분석했다. 두 시나리오를 대조한다.

**시나리오 1 — 필드명이 문서에 명시된 경우 (Figure 8: Tax Invoice)**

- **얕은 레이어**: attention이 필드 레이블 자체에 집중 → 명시적 단서로 의미적 앵커 확립
- **깊은 레이어**: attention이 레이블에서 대응하는 값 위치로 이동 → 추출 완료
- 추출 과정이 비교적 **직접적이고 국소적(localized)**

**시나리오 2 — 필드명이 문서에 존재하지 않는 경우 (Figure 9: Educational Field Trip Service Agreement)**

- **얕은 레이어**: attention이 정확한 위치가 아닌 의미적으로 관련된 구절들에 분산 → 가능한 앵커를 탐색(hypothesize and probe)
- **깊은 레이어**: attention이 단일 위치로 수렴하지 않고, 값을 포함할 가능성이 있는 **여러 후보 구간에 넓게 분산** 유지
- 모델이 **암묵적 후보 집계(candidate aggregation)와 비교(comparison)**를 통해 맥락 추론으로 정답 선택

| | 명시적 필드 단서 | 암묵적 필드 단서 |
|---|---|---|
| 얕은 레이어 | 필드 레이블에 집중 | 관련 구절들에 분산 |
| 깊은 레이어 | 대응 값으로 이동 | 여러 후보에 분산 유지 |
| 추출 방식 | 직접적·국소적 | 문맥 추론 기반 |

명시적 단서가 없으면 깊은 레이어의 attention이 수렴하지 못해 오류 가능성이 높아진다는 것이 핵심 발견이다.
